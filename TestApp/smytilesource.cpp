...
SMyTileSource::SMyTileSource(SMyTileType tileType) :
...
SMyTileSource::~SMyTileSource()
{
    qDebug() << this << this->name() << "Destructing";
}

QPointF SMyTileSource::ll2qgs(const QPointF &ll, quint8 zoomLevel) const
{
...
QPointF SMyTileSource::qgs2ll(const QPointF &qgs, quint8 zoomLevel) const
{
    const qreal tilesOnOneEdge = pow(2.0,zoomLevel);
    const quint16 tileSize = this->tileSize();
...
quint64 SMyTileSource::tilesOnZoomLevel(quint8 zoomLevel) const
{
    return pow(4.0,zoomLevel);
}

quint16 SMyTileSource::tileSize() const
{
    return 256;
}

quint8 SMyTileSource::minZoomLevel(QPointF ll)
{
    Q_UNUSED(ll)
    return 9;
}

quint8 SMyTileSource::maxZoomLevel(QPointF ll)
{
    Q_UNUSED(ll)
    return 14;
}

QString SMyTileSource::name() const
{
    switch(_tileType)
    {
    case SMyTiles:
...
QString SMyTileSource::tileFileExtension() const
{
...
QString SMyTileSource::urlFormat()
{
    return "/Gansu_jiayuguan&collection=ImgTile&tileKeyField=Key&dataField=Data&dataType=jpg_%1_%2_%3.jpg";
}

//protected
void SMyTileSource::fetchTile(quint32 x, quint32 y, quint8 z)
{
...
//private slot
void SMyTileSource::handleNetworkRequestFinished()
{
